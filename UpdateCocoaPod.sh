...
currentVersionString=`grep -E 's.version.*=' ZKCategories.podspec`
...
lineNumber=`grep -nE 's.version.*=' ZKCategories.podspec | cut -d : -f1`

sed -i "" "${lineNumber}s/${currentVersion}/${newVersion}/g" ZKCategories.podspec
