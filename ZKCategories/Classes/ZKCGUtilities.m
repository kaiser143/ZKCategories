...
CGContextRef ZKCGContextCreateARGBBitmapContext(CGSize size, BOOL opaque, CGFloat scale) {
...
CGContextRef ZKCGContextCreateGrayBitmapContext(CGSize size, CGFloat scale) {
    size_t width = ceil(size.width * scale);
    size_t height = ceil(size.height * scale);
    if (width < 1 || height < 1) return NULL;
...
CGAffineTransform ZKCGAffineTransformGetFromPoints(CGPoint before[3], CGPoint after[3]) {
...
CGAffineTransform ZKCGAffineTransformGetFromViews(UIView *from, UIView *to) {
    ...
    return ZKCGAffineTransformGetFromPoints(before, after);
}
