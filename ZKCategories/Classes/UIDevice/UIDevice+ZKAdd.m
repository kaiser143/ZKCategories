...
static uint64_t kai_net_counter_get_by_type(kai_net_interface_counter *counter, ZKNetworkTrafficType type) {
    ...
    if (type & ZKNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & ZKNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & ZKNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & ZKNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}
